...
}`))}function $t(t){return t.pattern.map(e=>Math.round(e*t.pixelRatio))}function hr(t){if(t==null)return 1;const e=$t(t);return Math.floor(e.reduce((i,r)=>i+r))}function fr(t){return $t(t).reduce((e,i)=>Math.max(e,i))}function mr(t){return t==null?Zt:t.length===4?t:Xt(vr,t[0],t[1],t[2],1)}const vr=qt();function gr(t,e){if(!e.stippleEnabled)return void t.fragment.code.add(p`float getStippleAlpha() { return 1.0; }
...
}`),a.uniforms.add(new hi("stipplePatternTexture",s=>s.stippleTexture),new Z("stipplePatternSDFNormalizer",s=>Sr(s.stipplePattern)),new Z("stipplePatternPixelSizeInv",s=>1/wt(s))),e.stippleOffColorEnabled&&a.uniforms.add(new Be("stippleOffColor",s=>mr(s.stippleOffColor))),a.code.add(p`float getStippleSDF(out bool isClamped) {
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`),a.uniforms.add(new Z("stipplePatternPixelSize",v=>wt(v))),a.main.add(p`float patternLength = lineSize * stipplePatternPixelSize;
